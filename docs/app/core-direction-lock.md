...
76. Oracle control-plane runtime may initialize and update local SQLite scheduler state behind `ORACLE_CONTROL_PLANE_*` env flags, and `primary` mode may own `all_active_subscriptions` enqueue admission, cadence timing, external-trigger ownership, and batch selection only; Oracle queue-control may also own empty-claim backoff/cooldown decisions behind `ORACLE_QUEUE_*` env flags, and Oracle queue-sweep control may own queued-worker tier cadence, sweep selection, and tier batch sizing behind `ORACLE_QUEUE_SWEEP_*` env flags. Supabase must remain the durable queue truth for leases, checkpoints, reservations/intents, queued/running rows, and user-facing writes until a later explicit migration phase moves those domains too.
