...
49. Supabase edge functions that the browser calls directly must echo the requesting allowed web origin (`https://bleup.app`, `https://www.bleup.app`, local dev, and explicit preview origins when intentionally enabled) and must not default browser CORS responses to one stale preview site.
