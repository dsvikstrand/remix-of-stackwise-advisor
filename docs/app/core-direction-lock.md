...
38. Subscription rows include `auto_unlock_enabled` (default `true`) and only new incoming subscription videos can auto-attempt unlock generation; runtime auto billing is funded-subscriber shared-cost with participant snapshotting, fixed-point funded-subset selection, bounded retries, and admin-bypass-safe funding. Subscribe/reactivate may still backfill up to the latest `5` creator videos into sparse Home `For You` walls (`<20` visible cards), but those historical rows stay locked unless a reusable ready blueprint already exists for that source item.
