...
38aa. Feed upgrades must preserve original wall arrival ordering: when an existing `user_feed_items` row upgrades from locked/unlockable to generated/published, its original `created_at` stays intact so Home `For You` does not bump that card back to the top on later enrichment.
