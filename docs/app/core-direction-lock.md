...
38. Subscription rows include `auto_unlock_enabled`; new subscriptions default it to `false`, reactivations preserve the prior saved value, and only new incoming subscription videos can auto-attempt unlock generation when the toggle is enabled. Runtime auto billing is funded-subscriber shared-cost with participant snapshotting, fixed-point funded-subset selection, bounded retries, and admin-bypass-safe funding. Subscribe/reactivate may still backfill up to the latest `5` creator videos into sparse Home `For You` walls (`<20` visible cards), but those historical rows stay locked unless a reusable ready blueprint already exists for that source item.
38a. Subscription sync should now prefer earlier wall visibility for newly discovered uploads: once a new source item is confirmed, Home `For You` may receive the ready blueprint row immediately when reusable output already exists, or the locked/unlockable row before later auto-unlock completion upgrades it.
38b. Repeated subscription feed `404` misses should attempt bounded creator recovery before being treated as stale: stored channel URL recovery remains first, and a creator-title lookup may repair the subscription only when it yields one strong canonical winner; weak or ambiguous name matches must not rewrite subscription identity.
38aa. Home `For You` now uses dual wall clocks for `user_feed_items`: locked/unlockable cards keep their original `created_at` wall-arrival time, while the first locked -> generated/published promotion stamps a separate `generated_at_on_wall` so the generated blueprint resurfaces once at its completion time.
38ab. Later enrichment on an already-generated `user_feed_items` row must not keep refreshing that generated wall clock; only the first locked -> generated/published transition may republish the card to the top.
...
38c. Queue-ledger `primary` compatibility writes must update existing Supabase `ingestion_jobs` rows by durable id first and only insert on a real miss; existing queue transitions should not default to `on_conflict` upserts.
...
47. Source-item truth may be staged behind the Oracle durable source-item ledger (`ORACLE_SOURCE_ITEM_LEDGER_MODE=supabase|dual|primary`), but Supabase remains the compatibility shadow until that ledger is explicitly promoted; user-facing source/feed behavior must stay consistent across both stores during the rollout, and any Oracle-side source-item read regression must fail safely back to `supabase` mode before re-staging.
   ...
   - In source-item-ledger `primary`, empty Oracle results for hot source-item reads are authoritative in the normal runtime path, so by-id, by-canonical-key, by-video-id, and batch source-item hydration should not reread Supabase just because Oracle found no matching row.
   - Supabase source-item compatibility writes should stay egress-conscious in that mode: unchanged shadow rows now skip no-op updates, `updated_at`-only source-item churn should stay Oracle-local instead of triggering a Supabase compatibility patch, normal compatibility writes should update by durable `id` before any canonical-key conflict fallback instead of eagerly rereading Supabase `source_items` by `id` + `canonical_key`, that shared update/insert seam should reuse the same mapped payload helper on both paths, and any remaining normal-path rereads should emit `source_item_fallback_read` so attribution can distinguish true fallback traffic from the Oracle-first baseline.
...
48. Generation execution truth may now also be staged behind the Oracle durable generation-state ledger (`ORACLE_GENERATION_STATE_MODE=supabase|dual|primary`), but Supabase remains the compatibility shadow until that ledger is explicitly promoted; variant claim/ready/failed state plus `generation_runs` summary reads must stay consistent across both stores during rollout, and any Oracle-side generation-state regression must fail safely back to Supabase-backed readers/writers before re-staging.
   - This chapter intentionally covers `source_item_blueprint_variants` plus `generation_runs`, while `generation_run_events` remain on Supabase for now.
   - Source-page unlock, search/manual generate, and shared blueprint-availability cooldown reads must prefer Oracle generation-state rows in `primary` while keeping narrow fallback only for explicit resilience cases.
   - Generation-state `dual` parity must compare variants by logical key (`source_item_id + generation_tier`) rather than backend-local UUID alone, and should treat normalized empty `quality_issues` plus sub-second timestamp skew as benign shadow representation differences rather than blocking drift.
49. Once Oracle queue-ledger runtime is in `primary`, normal queue-status reads must treat an empty Oracle result as authoritative for the hot path rather than automatically re-querying Supabase for the same owner/scope/latest lookup.
   - Remaining queue misses in that mode are Oracle-authoritative and should emit `queue_oracle_only_bypass` rather than silently rereading Supabase.
   - Lease-heartbeat-only queue touches should not emit a full Supabase `ingestion_jobs` shadow upsert on every refresh; compatibility shadow churn should stay focused on enqueue/claim/terminal transitions that materially change durable queue state.
50. Supabase edge functions that the browser calls directly must echo the requesting allowed web origin (`https://bleup.app`, `https://www.bleup.app`, local dev, and explicit preview origins when intentionally enabled) and must not default browser CORS responses to one stale preview site.
...
51. Subscription sync persistence is write-throttled for backend efficiency: unchanged successful sync writes to `user_source_subscriptions` should be skipped unless checkpoint/title/error state changes, while repeated identical error writes remain bounded by the `30m` poll heartbeat; user-facing subscription health remains a `60m` UX window.
   ...
   - subscription-ledger `primary` reads must treat empty Oracle results as authoritative for hot subscription state checks (`by id`, `by user + channel`, source-page access, per-user lists/counts) rather than automatically re-querying Supabase for the same lookup.
   - remaining Supabase subscription reads in that mode are explicit compatibility fallbacks only and should emit `subscription_fallback_read`.
   - compatibility shadow writes should skip no-op `user_source_subscriptions` patches when the meaningful persisted fields already match.
...
49f. Current live subscription-discovery tuning now uses a tighter Oracle-primary cadence for fast creator uploads: `ORACLE_SUBSCRIPTION_PRIMARY_MIN_TRIGGER_INTERVAL_MS=300000` and `ORACLE_SUBSCRIPTION_REVISIT_ACTIVE_MS=300000` on Oracle, while normal/quiet revisit windows remain wider.
...
49j. Queue-ledger `primary` compatibility shadowing should now stay selective: enqueue/retry/terminal transitions may still shadow `ingestion_jobs` into Supabase, but claim-to-`running` transitions should not emit a second compatibility upsert when Oracle already owns the live worker state.
...
49n. Rapid back-to-back interactive generation requests should refill free worker capacity promptly: if one interactive job is already running and another worker slot is still open, later interactive jobs must not wait for the full prior claim batch to drain before they can be claimed.
...
52a. `generation_started` inbox rows must emit per queued job again; unread start notifications may dedupe repeated emits for the same `jobId`, but they must not coalesce distinct rapid-fire jobs into one scope-wide start item.
...
64. Creator lookup should also behave as bounded find-this-one flow: channel URL, handle, and channel id are preferred inputs; creator-name lookup is helper-backed, returns only a tiny candidate set, and must not depend on official `search.list`.
   - Manual `Add Subscription` creator lookup should expose explicit input modes (`Handle`, `Creator name`, `Channel URL / ID`) instead of guessing user intent from one free-text box.
   - Explicit `Handle` mode should prefer official YouTube `forHandle` resolution before any legacy HTML handle-page scraping fallback, so valid handles such as `@DaveAspreyBPR` do not depend on brittle page HTML extraction.
...
72. The default one-step YT2BP prompt contract is now `docs/golden_blueprint/golden_bp_prompt_contract_one_step_v6.md`: it keeps the same `blueprint_sections_v1` runtime shape, but `Takeaways` should bias toward plain-English fast-skim value, `Storyline` should stay at `2-3` substantial paragraphs/slides rather than thin fragments, long transcript pruning should be treated as normal runtime shaping rather than a caveat trigger, and the existing `open_questions` field should carry a more reader-useful `Caveats` section built around balancing nuance instead of repetitive evidence-policing.
...
76. Oracle control-plane runtime may initialize and update local SQLite scheduler state behind `ORACLE_CONTROL_PLANE_*` env flags, and `primary` mode may own `all_active_subscriptions` enqueue admission, cadence timing, external-trigger ownership, and batch selection only; Oracle queue-control may also own empty-claim backoff/cooldown decisions behind `ORACLE_QUEUE_*` env flags, Oracle queue-sweep control may own queued-worker tier cadence, sweep selection, and tier batch sizing behind `ORACLE_QUEUE_SWEEP_*` env flags, Oracle queue-admission mirror state may serve hot queue-depth/backpressure reads behind `ORACLE_QUEUE_ADMISSION_*` env flags, and Oracle job-activity mirror state may serve Oracle-first stale-running recovery, manual-refresh duplicate guards, retry/refresh pending-job dedupe, unlock-reliability job lookups, active `all_active_subscriptions` duplicate guards, owner-scoped `GET /api/ingestion/jobs/:id`, Oracle-backed queue-position reads for `GET /api/ingestion/jobs/active-mine`, and user/ops latest-job status reads behind `ORACLE_JOB_ACTIVITY_*` env flags.
   ...
   - Oracle-primary queue retry requeues may now also stay Oracle-local when a claimed job is rescheduled back to `queued`; Supabase `ingestion_jobs` should not be patched back to a retry state unless a later compatibility requirement proves it is still needed.
   - The remaining queue compatibility writes should now also be action-class aware: terminal shadows may stay conservative for rollback safety, but retry/claim/lease classes should be classified explicitly and any Supabase `PATCH /ingestion_jobs` shadow should send only the fields relevant to that lifecycle class instead of restamping the full queue row.
   - Remaining Oracle-only queue bypasses should log `queue_oracle_only_bypass` consistently across latest-for-user, active-for-user, refresh-pending dedupe, unlock job lookup, and retry-dedupe paths so diagnostics can still spot unexpected misses without rereading Supabase.
   - Current queue cutover direction is explicit: full Oracle queue ownership is the end state, and queue-ledger `primary` now runs as Oracle-only normal runtime rather than a mixed shadow model.
   - Oracle-only queue also means Supabase `source_item_unlocks` compatibility rows must not persist queue `job_id` values that still foreign-key to Supabase `ingestion_jobs`; Oracle unlock/product mirrors keep that operational job ownership instead.
   ...
   - In subscription-ledger `primary`, empty Oracle results for hot subscription reads are authoritative in the normal runtime path, so by-id, by-user-plus-channel, per-user active-list, due-batch hydration, source-page subscriber fan-out, and auto-unlock eligibility reads should not reread Supabase just because Oracle found no matching row.
   - Supabase subscription compatibility writes should stay egress-conscious in that mode: normal shadow updates should write by durable subscription `id` first, only fall back to user/channel lookup on a rare miss, unchanged rows should skip no-op updates, and remaining rereads should emit `subscription_fallback_read` so attribution can separate true fallback traffic from the Oracle-first baseline.
...
12. Deterministic long-transcript pruning now defaults to a `5000`-character budget with threshold buckets `5000/9000/16000`; over-budget transcripts still use the same evenly spaced window sampler before the final hard trim.
