...
49e. `all_active_subscriptions` breadth must stay bounded but Oracle-tunable: the runtime may still process the full corpus eventually, but each run should prioritize Oracle-local due rows first and cap the per-run slice through `ORACLE_SUBSCRIPTION_PRIMARY_BATCH_LIMIT` (`150` current default in `primary`) so subscription refresh cost and backlog clearance remain explicit control-plane choices.
...
76. Oracle control-plane runtime may initialize and update local SQLite scheduler state behind `ORACLE_CONTROL_PLANE_*` env flags, and `primary` mode may own `all_active_subscriptions` enqueue admission, cadence timing, external-trigger ownership, and batch selection only; Supabase must remain the durable queue truth for leases, checkpoints, reservations/intents, and user-facing writes until a later explicit migration phase moves those domains too.
