...
49l. Fresh `source_item_unlocks` rows must initialize `transcript_status='unknown'`; subscription sync must not insert unlock rows with null transcript transcript-truth fields, because that now breaks cron progress on the durable unlock table contract.
