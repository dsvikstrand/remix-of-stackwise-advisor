...
d9e) [have] Oracle runtime now also stages durable source-item truth for `source_items` behind `ORACLE_SOURCE_ITEM_LEDGER_MODE`, with Oracle-aware source-item upserts, metadata/view-count updates, and Oracle-first execution/feed read paths using that durable ledger in `dual|primary` while Supabase remains the compatibility shadow.
