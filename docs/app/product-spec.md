...
a58a) [have] Service-cron subscription ingestion still triggers every `3m`, but backend enqueue now gates `all_active_subscriptions` to an effective minimum interval of `10m` by default, reducing background requeue churn without changing active-work queue behavior.
