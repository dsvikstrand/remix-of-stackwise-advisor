...
- Notifications ownership is in staged Oracle cutover:
  - notification reads and writes are now Oracle-first, while Supabase `notifications` remains a compatibility shadow only for the current push-enqueue path.
...
a19b) [have] `youtube_timedtext` is opportunistic: expected native-caption misses (`NO_CAPTIONS`, `TRANSCRIPT_EMPTY`) fall through to fallback providers without incrementing provider-circuit failure counts, while real timedtext provider trouble still records circuit failures.
...
a58a) [have] Service-cron subscription ingestion still triggers every `3m`, but backend enqueue now gates `all_active_subscriptions` through the Oracle cadence window (`ORACLE_SUBSCRIPTION_PRIMARY_MIN_TRIGGER_INTERVAL_MS`, default `5m`), reducing duplicate background requeue churn while still keeping new-upload discovery materially faster than the old hourly baseline.
...
a58l) [have] Queue runtime is now fully in the Oracle-only ownership chapter: queue-ledger `primary` is the normal enqueue/claim/lease/retry/terminal/status system, and the older Supabase queue compatibility toggle is no longer part of steady-state runtime.
...
a61ca) [have] Source-item ownership Pass 1 now also removes Supabase `source_items` from Oracle restart/bootstrap input in `primary`: the Oracle product/source mirrors rebuild recent source rows from `source_item_ledger_state` rather than rereading Supabase source rows during normal startup.
a61cb) [have] Source-item ownership Pass 2 now also removes normal-runtime Supabase `source_items` writes in `primary`: shared source-item insert/upsert and metadata/view-count updates now write Oracle source-item ledger + Oracle product-source mirror only.
a61cc) [have] Source-item ownership Pass 3 now also removes the main normal-runtime Supabase `source_items` reads in `primary`: `/api/my-feed` source hydration, feed accept/source lookup helpers, and browser-side blueprint/profile source attribution now resolve through Oracle-aware server readers instead of direct Supabase `source_items` reads.
...
a61e) [have] Oracle unlock ownership now also cuts Supabase out of unlock bootstrap input: once unlock-ledger `primary` is live, Oracle restart/bootstrap should keep unlock truth from the Oracle unlock ledger itself, and the Oracle product unlock mirror should rebuild from that Oracle-owned state rather than rehydrating stale `source_item_unlocks` shadow rows.
a61f) [have] The later unlock closure also removed the remaining meaningful active-runtime product/browser reads of `source_item_unlocks`: Blueprint Detail source attribution, liked-blueprint profile source linkage, and the browser-side legacy `/api/my-feed` fallback no longer read Supabase unlock rows directly.
...
a86) [have] Notifications ownership is now in staged Oracle cutover: notification reads, write-side rows, and read-state mutations land Oracle-first, while Supabase `notifications` remains only as the temporary compatibility shadow for the current push-dispatch enqueue path.
...
a86b) [have] Queue-ledger primary now mirrors existing `ingestion_jobs` compatibility rows by durable-id update with insert-on-miss fallback, reducing Supabase `on_conflict` POST churn without changing queue behavior.
a86c) [have] Wall and Home `For You` liked-state hydration now uses the backend Oracle-backed blueprint-like reader/API; feed service code no longer falls back to Supabase `blueprint_likes` for normal runtime viewer-liked enrichment.
...
a91a) [have] When backend API access is unavailable or unset, the browser-side legacy `My Feed` fallback now uses only the last cached snapshot; it no longer reconstructs unlock state by querying Supabase `source_item_unlocks` directly.
...
a92f) [have] Tag read ownership now also covers browser product surfaces: popular/featured/suggested/trending tags, Explore tag lookup, Channels preview tag hydration, legacy post-detail tag lookup, and `/api/my-feed` blueprint tag hydration resolve through backend Oracle-aware tag APIs instead of direct browser reads from Supabase `tags`, `blueprint_tags`, or `tag_follows`.
a92g) [have] Main backend tag write ownership now also lands on Oracle: backend tag creation mints IDs in Oracle `tag_state`, tag follow/unfollow updates Oracle `tag_follow_state` plus Oracle follower counts, and backend blueprint creation / auto-channel / manual channel publish use the shared Oracle-aware tag-id helper before attaching Oracle `blueprint_tag_state` rows.
a92e) [have] Provider-circuit-state ownership now keeps normal runtime circuit reads and writes on Oracle control-plane state in `primary`: provider success/failure/open/close mutations persist to local Oracle-backed `provider_circuit_state`, and provider-availability reads plus `GET /api/ops/queue/health` snapshots now resolve from that Oracle-backed state as the normal path.
...
- Oracle control-plane subscription scheduler migration remains additive:
  ...
  - Oracle-primary drain breadth is now separately configurable through `ORACLE_SUBSCRIPTION_PRIMARY_BATCH_LIMIT`, and one run may drain multiple due batches through `ORACLE_SUBSCRIPTION_PRIMARY_MAX_BATCHES_PER_RUN`; the current live rollout uses `150` rows and `2` batches with a `5m` cadence override plus `5m` active revisit, improving backlog clearance and new-upload discovery without moving queue truth off Supabase.
  ...
  - Oracle-primary queue retry requeues may now also stay Oracle-local when a claimed job is rescheduled back to `queued`; the Supabase compatibility row is no longer patched just to mirror the retry state transition.
  - The remaining queue compatibility patches are now also lifecycle-aware: queue shadow updates classify terminal vs retry/claim/lease behavior and send only the fields needed for that class, reducing how much Supabase still participates in the normal queue write path without changing visible queue UX.
  - In queue-ledger `primary`, normal queue read misses are now Oracle-local authoritative misses rather than a signal to reread Supabase.
  ...
  - Oracle durable feed envs (`ORACLE_FEED_LEDGER_MODE`, `ORACLE_FEED_LEDGER_BOOTSTRAP_LIMIT`) may now also bootstrap and maintain a local SQLite ledger for `user_feed_items`: `dual` shadows feed insert/update/delete transitions on Oracle while still keeping the Supabase row shadow, and `primary` lets wall/profile/public feed readers plus shared feed mutation paths prefer Oracle-backed feed rows while Supabase becomes migration residue instead of the normal feed write target.
  - In feed-ledger `primary`, Oracle restart/bootstrap should no longer repopulate durable feed truth from Supabase `user_feed_items`; feed/product bootstrap now rebuilds feed rows from Oracle `feed_ledger_state` instead of treating Supabase feed rows as authoritative input.
  - Feed ownership Pass 2 also removes the main Oracle-primary `user_feed_items` insert/upsert shadow writes; Oracle feed mutations now persist to `feed_ledger_state` + `product_feed_state`, while the remaining read-side cutover is handled in later feed passes.
  - Feed ownership Pass 3 now removes the main Oracle-primary `user_feed_items` read fallbacks as well: wall/public/my-feed/product feed readers treat Oracle feed ledger + Oracle product feed state as the normal read path, and browser-side existing-feed-item checks prefer the backend-shaped Oracle-aware `/api/my-feed` payload before any legacy Supabase fallback.
  - Feed ownership follow-up now also makes source-video feed-row existence checks Oracle-authoritative in `primary`: when `feed_ledger_state` has no row for `user_id + source_item_id`, the backend treats that as a real miss instead of rereading Supabase `user_feed_items`; Oracle product feed mirror remains an Oracle-local secondary read only.
  - Oracle durable generation-state envs (`ORACLE_GENERATION_STATE_MODE`, `ORACLE_GENERATION_STATE_BOOTSTRAP_LIMIT`) may now also bootstrap and maintain local SQLite execution truth for `source_item_blueprint_variants` plus `generation_runs`: `dual` shadows variant/run writes on Oracle while still keeping the Supabase shadow, and `primary` lets shared variant ownership/readiness and generation-run summary reads prefer Oracle-backed execution state while the adjacent trace-events chapter severs `generation_run_events`.
  - Generation-state Pass 1 now also removes Supabase generation-state rehydration from Oracle bootstrap in `primary`: restart/bootstrap should keep execution truth from `generation_variant_state` and `generation_run_state`, and startup count reporting should come from those Oracle-owned tables instead of rereading Supabase `source_item_blueprint_variants` or `generation_runs`.
  - Generation-state Pass 2 now also removes normal-runtime Supabase generation-state shadow writes in `primary`: variant claim/ready/failed transitions plus generation-run lifecycle writes should stay Oracle-only, with Supabase `source_item_blueprint_variants` and `generation_runs` no longer needed for normal mutation correctness.
  - Generation-state Pass 3 now also removes the main normal-runtime Supabase generation-state read fallbacks in `primary`: source-page duplicate/in-progress/ready checks, blueprint-availability cooldown reads, profile/detail generation attribution, and the shared variant/run readers now treat Oracle execution state as the normal runtime source instead of rereading Supabase on ordinary misses.
  - Generation-trace Pass 2 now also removes normal-runtime Supabase `generation_run_events` writes in `primary`: shared milestone/terminal event appends and per-run event sequencing now stay Oracle-only.
  - Generation-trace Pass 3 now also removes the main normal-runtime Supabase trace-event reads in `primary`: trace detail routes and shared event pagination now resolve Oracle event state instead of rereading Supabase `generation_run_events` on ordinary runtime misses.
...
f6c) Oracle-backed Home `For You` feed-window reads must apply that same effective wall timestamp before `limit` is applied (`generated_at_on_wall || created_at`), so generated-heavy accounts still receive older locked rows in the candidate window.
...
d9aa) [have] Under Oracle-only queue runtime, the Supabase `source_item_unlocks` compatibility row no longer mirrors queue `job_id` values; the real unlock job ownership remains on Oracle-ledger/product state so source unlock generation does not depend on a Supabase `ingestion_jobs` foreign key.
...
d9da) [have] In unlock-ledger `primary`, normal unlock reservation/processing/fail/ready mutations, expired-hold scans, transcript suppression/revalidate seed reads, and server-side legacy `My Feed` unlock hydration now stay on Oracle-owned unlock state instead of rereading or rewriting Supabase `source_item_unlocks`.
...
si52b) auto-unlock `NO_ELIGIBLE_USERS` is terminal for retry scheduling: the locked `my_feed_unlockable` card stays available for manual unlock, but no `source_auto_unlock_retry` job is queued unless eligible users exist and the remaining blocker is transient, such as credit availability, transcript cooldown, or queue backpressure.
...
si53c) `GET /api/ops/queue/health` also exposes explicit worker-activity semantics: legacy `worker_running` means fresh running-job activity exists, while `worker_activity_state`, `worker_activity_reason`, `worker_running_semantics`, and `worker_service_liveness_source` prevent operators from mistaking an idle split-worker queue for a stopped `agentic-worker.service`.
...
si73) blueprint create/update/save routes are backend-owned for current product flows: the browser calls `/api/blueprints*`, while the backend writes the needed Supabase compatibility row and Oracle `blueprint_state` / `blueprint_tag_state` ownership state. Direct browser Supabase writes to `blueprints` or `blueprint_tags` are legacy residue, not the normal save path.
si74) blueprint list/search/count reads are backend-owned for current browser surfaces: search, Explore, suggested blueprints, community stats, Channels previews, landing samples, and My Feed section lookup use `/api/blueprints` or blueprint detail routes backed by Oracle `blueprint_state` rather than browser Supabase `blueprints`.
si75) tag-family backend fallback is explicit break-glass only: auto-channel, manual channel publish, and generated-blueprint tag attachment require Oracle-aware tag owner dependencies, and Supabase `tags` / `blueprint_tags` fallback is disabled by default unless `ORACLE_TAG_FAMILY_SUPABASE_BREAK_GLASS_ENABLED` is intentionally enabled.
