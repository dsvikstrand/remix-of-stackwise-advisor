...
a58g) [have] Durable unlock truth now also requires fresh `source_item_unlocks` rows to initialize `transcript_status='unknown'`; leaving that field null is now treated as a live-breaking bug because subscription sync inserts use the same unlock table contract.
...
a58i) [have] Source-video unlock throttling now uses soft request caps (burst+sustained) instead of hard cooldown, and frontend credit meter refreshes immediately after unlock actions.
