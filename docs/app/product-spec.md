...
- Oracle control-plane subscription scheduler migration remains additive:
  ...
  - Oracle queue-control envs (`ORACLE_QUEUE_*`) may now also move empty-claim cooldown and tier-aware claim backoff into Oracle-local SQLite/in-process state, while Supabase still owns queued/running rows, claim/lease truth, and retries.
