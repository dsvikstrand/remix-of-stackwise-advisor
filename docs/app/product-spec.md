...
- Oracle control-plane subscription scheduler migration remains additive:
  ...
  - Generation-trace Pass 2 now also removes normal-runtime Supabase `generation_run_events` writes in `primary`: shared milestone/terminal event appends and per-run event sequencing now stay Oracle-only, while trace detail reads remain the next cut.
