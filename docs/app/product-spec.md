...
- Oracle control-plane subscription scheduler migration remains additive:
  ...
  - Oracle queue-control envs (`ORACLE_QUEUE_*`) may now also move empty-claim cooldown and tier-aware claim backoff into Oracle-local SQLite/in-process state, and Oracle queue-sweep envs (`ORACLE_QUEUE_SWEEP_*`) may now make Oracle authoritative for queued-worker tier cadence, sweep selection, and tier batch sizing. Supabase still owns queued/running rows, claim/lease truth, and retries.
