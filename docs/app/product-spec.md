...
- Oracle control-plane subscription scheduler migration remains additive:
  ...
  - Oracle queue-control envs (`ORACLE_QUEUE_*`) may now also move empty-claim cooldown and tier-aware claim backoff into Oracle-local SQLite/in-process state, Oracle queue-sweep envs (`ORACLE_QUEUE_SWEEP_*`) may now make Oracle authoritative for queued-worker tier cadence, sweep selection, and tier batch sizing, and Oracle queue-admission mirror envs (`ORACLE_QUEUE_ADMISSION_*`) may now let hot admission/preflight paths read Oracle-local active queue counts instead of repeatedly querying Supabase for the same queue-depth snapshot. Supabase still owns queued/running rows, claim/lease truth, and retries.
