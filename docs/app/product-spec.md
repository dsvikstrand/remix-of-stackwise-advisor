...
a58l) [have] Queue runtime is now in the explicit Oracle-only cutover chapter: queue-ledger `primary` should default to Oracle-only enqueue/claim/lease/retry/terminal/status behavior, while `ORACLE_QUEUE_SUPABASE_COMPAT_ENABLED=true` is kept only as a short-lived rollback lever during burn-in.
