...
- 2026-04-07 note: queue-ledger `primary` now also supports Oracle-only queue cutover mode. Normal queue writes and normal queue read misses may stay Oracle-local by default, while `ORACLE_QUEUE_SUPABASE_COMPAT_ENABLED=true` is reserved as the temporary rollback lever for restoring the older Supabase queue compatibility path. This is additive backend control-plane behavior outside the YT2BP request/response envelope.
