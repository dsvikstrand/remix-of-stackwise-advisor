...
- 2026-04-01 note: Oracle queue-ledger ownership may now also be staged behind `ORACLE_QUEUE_LEDGER_MODE=supabase|dual|primary`, with `dual` bootstrapping/shadowing a local durable queue ledger and `primary` reserved for a later live claim/lease/fail/finalize cutover. This is additive backend control-plane behavior and does not alter the YT2BP request/response envelope.
