...
- 2026-04-29 note: subscription auto-unlock retry scheduling now treats `NO_ELIGIBLE_USERS` as terminal/no-op while preserving manual locked cards and bounded retries for transient blockers such as credits, transcript cooldown, and queue pressure. This remains outside this endpoint envelope.
...
- 2026-04-15 note: notification ownership is now in staged Oracle cutover. Notification reads, writes, and read-state mutations are Oracle-first, while Supabase `notifications` remains as temporary compatibility shadow only for the current push-enqueue path. This is additive backend behavior outside the YT2BP request/response envelope.
- 2026-05-06 note: Wall and Home `For You` blueprint-like state now resolves through the backend Oracle-backed like-state reader/API, and normal feed hydration no longer falls back to Supabase `blueprint_likes`. This is additive product-read ownership behavior outside the YT2BP request/response envelope.
- 2026-05-08 note: backend tag creation, tag follow/unfollow, and backend blueprint-tag assignment paths now use Oracle tag state as the normal owner before attaching `blueprint_tag_state` rows. This is additive ownership behavior outside the YT2BP request/response envelope.
...
- 2026-04-15 note: provider circuit bookkeeping in `primary` now keeps both mutations and normal read-side availability/ops snapshots on Oracle-local `provider_circuit_state`, with ordinary Supabase rereads removed from the steady-state control path. This is additive backend control-plane behavior and does not alter the YT2BP request/response envelope.
...
- 2026-05-16 note: Oracle feed-ledger `primary` now also owns source-video feed-row existence checks. Empty Oracle `feed_ledger_state` results for `user_id + source_item_id` are authoritative, so subscription/search/manual/source-page generation flows no longer reread Supabase `user_feed_items` just to decide whether a feed row already exists. This is additive ownership cleanup and does not alter the YT2BP request/response envelope.
...
- 2026-05-03 note: Oracle feed-ledger/product-feed read windows now apply that effective wall-display timestamp before `limit`, preventing generated-heavy personal feeds from excluding older locked rows before final Home `For You` sorting. This is additive feed-read behavior and does not alter the YT2BP request/response envelope.
...
- 2026-04-30 note: service queue-health now adds worker-activity semantics fields so `worker_running` is interpreted as fresh running-job activity rather than worker service liveness; this is ops metadata only and remains outside this endpoint envelope.
...
- 2026-04-10 note: main backend blueprint-tag ownership now writes tag joins into Oracle `blueprint_tag_state` for blueprint creation and channel publish flows. Bootstrap seeds existing joins there, and current runtime tag reads for wall/channel/source-page/search-oriented surfaces resolve through Oracle-backed rows instead of direct Supabase `blueprint_tags`. This is additive backend product-ledger behavior and does not alter the YT2BP request/response envelope.
- 2026-05-07 note: browser tag-read ownership now follows the same backend boundary: frontend product surfaces resolve tag directory, tag lookup, followed tags, and blueprint-tag joins through `/api/tags*` and `/api/blueprint-tags` instead of direct Supabase `tags`, `blueprint_tags`, or `tag_follows` reads. This is additive product-read behavior and does not alter the YT2BP request/response envelope.
...
- Service queue operations (`POST /api/ingestion/jobs/trigger`, `GET /api/ingestion/jobs/latest`, `GET /api/ops/queue/health`) are intentionally outside this endpoint contract. Queue-health worker activity/liveness-source fields are ops-only metadata.
