...
- 2026-03-31 note: Oracle control-plane bootstrap/shadow groundwork adds local SQLite scheduler state/envs (`ORACLE_CONTROL_PLANE_*`) for subscription-scheduler migration; active YouTube subscriptions can now bootstrap locally and shadow decisions can be persisted for comparison, while Supabase remains authoritative for durable queue truth and live enqueue decisions outside the YT2BP request/response envelope.
