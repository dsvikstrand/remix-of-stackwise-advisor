...
- 2026-03-31 note: Oracle control-plane bootstrap foundation adds default-off local SQLite scheduler state/envs (`ORACLE_CONTROL_PLANE_*`) for future subscription-scheduler migration, while Supabase remains authoritative for durable queue truth; this is additive backend runtime groundwork outside the YT2BP request/response envelope.
