...
- 2026-04-15 note: provider circuit bookkeeping in `primary` now writes Oracle-local `provider_circuit_state` first for provider success/failure/open/close mutations; current availability reads and ops snapshots stay on the existing reader path for now. This is additive backend control-plane behavior and does not alter the YT2BP request/response envelope.
