...
- 2026-04-01 note: Oracle product-mirror envs may now also bootstrap/mirror active subscriptions, recent source items, source-item unlock rows, and recent feed rows into Oracle-local SQLite so source-page subscription/access checks and unlock-status lookups can prefer Oracle-local reads first; Supabase still remains the authoritative product ledger, and wall/profile feed hydration is intentionally still conservative. This is additive backend behavior and does not alter the YT2BP request/response envelope.
