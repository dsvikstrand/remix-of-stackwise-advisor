...
- 2026-04-02 note: Oracle durable source-item truth may now also be staged behind `ORACLE_SOURCE_ITEM_LEDGER_MODE=supabase|dual|primary`, with `dual` bootstrapping/shadowing `source_items` into a local Oracle ledger and `primary` now letting source-item upserts, metadata/view-count updates, execution-path source lookups, and Oracle-first wall/profile/source-page source-row reads prefer Oracle-backed source rows while Supabase remains the compatibility shadow. This is additive backend product-ledger behavior and does not alter the YT2BP request/response envelope.
