...
- 2026-04-15 note: notification ownership is now in staged Oracle cutover. Notification writes and read-state mutations are Oracle-first with Supabase compatibility shadowing so inbox reads and push enqueue stay stable. This is additive backend behavior outside the YT2BP request/response envelope.
