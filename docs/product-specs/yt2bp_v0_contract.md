...
- 2026-04-02 note: source-page unlock preparation failures should now surface as explicit prepare-failed outcomes rather than being reported back as generic `in_progress`, and Oracle-primary unlock mutation wrapper failures should fall back to the durable Supabase unlock mutation path plus Oracle shadow resync. This is additive backend resilience and outside the YT2BP endpoint envelope.
