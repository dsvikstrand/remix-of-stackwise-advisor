...
- `ORACLE_PRODUCT_BOOTSTRAP_LIMIT` (default `2000`; number of recent `source_items` and `user_feed_items` rows loaded from Supabase into the Oracle product mirror during bootstrap, while active subscriptions are merged in full and unlock rows rebuild from the Oracle unlock ledger)
