...
- Personal-first routing is now expected:
  ...
  - `llm_native` quality retries now stay focused on blocking structure/shape misses; `TAKEAWAYS_TOO_LONG` and `OPEN_QUESTIONS_NOT_QUESTIONS` still appear in logs/trace output, but they no longer trigger regeneration on their own.
