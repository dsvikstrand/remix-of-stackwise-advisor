...
- Current blueprint-tags cutover posture:
  ...
  - backend tag creation and follow/unfollow now use Oracle `tag_state` / `tag_follow_state` for the normal runtime path instead of Supabase `tags` / `tag_follows`.
...
- Oracle-only feed existence note: in `primary`, backend checks for an existing per-user source-video row (`user_id + source_item_id`) should treat an empty Oracle feed-ledger result as authoritative and must not reread Supabase `user_feed_items`; if the Oracle ledger read itself fails, investigate the Oracle path rather than hiding the miss through Supabase fallback.
...
- In `primary`, normal variant ownership/ready-state checks and generation-run summary reads should resolve from Oracle generation state first; the adjacent trace-events chapter now owns both runtime `generation_run_events` writes and reads.
...
- In `primary`, Oracle generation-trace writes should now also be Oracle-only for normal runtime event paths: shared milestone/terminal event appends and per-run `seq` allocation should not shadow Supabase `generation_run_events`.
- In `primary`, Oracle generation-trace reads should now also be Oracle-only for normal runtime trace detail paths: shared event pagination plus `/api/generation-runs/:runId` and `/api/blueprints/:id/generation-trace` should not reread Supabase `generation_run_events` on ordinary runtime misses.
...
```
  ...
  - Keep scope explicit:
    ...
    - `generation_run_events` write and read truth now also move in the later trace-events chapter
- YT2BP repro smoke:
```bash
