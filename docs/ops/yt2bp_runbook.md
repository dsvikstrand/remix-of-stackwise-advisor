...
- Source-page endpoints:
  ...
  - User ingestion-status routes are also narrower:
    ...
    - subscription feed fetch failures are now classified/hardened on the Oracle-primary path: transient YouTube feed `5xx/network` errors retry inside the sync before being downgraded to soft per-subscription outcomes, `404` feed failures try stale-channel recovery from the stored channel URL before backing off to a progressively quieter revisit interval for repeated misses (`1x` -> `2x` -> `4x` -> `8x`, capped at `24h`), and soft per-subscription feed failures no longer poison an otherwise healthy `all_active_subscriptions` batch
...
- `ORACLE_PRODUCT_MIRROR_ENABLED` (default `false`; enables Oracle-local mirrored product-state rows for subscriptions, source items, source-item unlocks, and recent feed rows so source-page access, blueprint-cooldown, unlock-status/trust checks, public wall/source-page blueprint feeds, and wall/profile feed-history reads can prefer Oracle-local reads when the mirror is sufficiently complete, with Supabase fallback underneath)
