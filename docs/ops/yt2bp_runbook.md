...
- `ORACLE_PRODUCT_MIRROR_ENABLED` (default `false`; enables Oracle-local mirrored product-state rows for subscriptions, source items, source-item unlocks, and recent feed rows so source-page access, unlock-status/trust checks, and wall/profile feed-history reads can prefer Oracle-local reads when the mirror is sufficiently complete, with Supabase fallback underneath)
