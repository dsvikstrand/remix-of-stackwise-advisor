...
- Source-page endpoints:
  ...
  - User ingestion-status routes are also narrower:
    ...
    - Oracle queue-admission and job-activity mirror bootstrap/refresh now also rebuild from that Oracle queue ledger first once it is live, so those Oracle-local mirrors do not need to re-bootstrap from Supabase during normal `primary` runtime
