...
a4t) [have] Active Supabase egress-reduction work now skips unchanged successful subscription sync writes (`user_source_subscriptions`) unless checkpoint/title/error state changes, while repeated identical error writes remain bounded by a `30m` backend heartbeat and `all_active_subscriptions` enqueue is gated to an effective `10m` minimum interval; treat that as current runtime truth even though detailed proof/next steps live in the active egress plan.
...
a4ya) [have] Queue-backed source-video generation now also records variant `active_job_id`, reclaims stale queued/running source-item variants after a bounded timeout, and persists terminal `generation_runs` status outside the best-effort trace-event wrapper so completed work does not remain stuck as `running`.
