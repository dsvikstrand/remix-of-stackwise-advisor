...
- Current runtime now also includes the first Oracle control-plane bootstrap foundation behind default-off env flags (`ORACLE_CONTROL_PLANE_*`): local SQLite scheduler state can be initialized for future subscription-scheduler migration, while Supabase still owns durable queue truth in the active MVP contract.
