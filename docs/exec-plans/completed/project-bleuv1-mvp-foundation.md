...
- Current runtime follow-up now also requires fresh durable unlock rows to initialize `transcript_status='unknown'`; subscription sync inserts that leave transcript truth null break the shared `source_item_unlocks` contract and stall new locked-feed creation.
