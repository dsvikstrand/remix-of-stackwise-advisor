...
- Current runtime follow-up now also removes Supabase feed rehydration from Oracle bootstrap in `primary`, so restart/bootstrap should keep feed truth from Oracle `feed_ledger_state` and Oracle product-feed bootstrap should source feed rows from that ledger instead of rereading Supabase `user_feed_items`.
