...
- Current runtime follow-up now also starts Oracle provider-circuit-state ownership in `primary`: normal success/failure/open/close mutations write local `provider_circuit_state` first, while provider-availability reads and ops snapshots remain on the earlier reader path until the follow-up read pass.
