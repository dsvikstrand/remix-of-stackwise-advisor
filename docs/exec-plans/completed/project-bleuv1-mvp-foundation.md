...
- Current runtime follow-up now also keeps Home wall ordering stable across that upgrade path: if an existing locked/unlockable feed row later receives a blueprint, its original `created_at` is preserved so the card does not jump back to the top.
