...
- Current runtime now also includes Oracle control-plane subscription-scheduler groundwork behind `ORACLE_CONTROL_PLANE_*` env flags: local SQLite scheduler state can bootstrap active YouTube subscriptions, record shadow scheduler decisions, and in `primary` own `all_active_subscriptions` enqueue admission plus batch selection, while Supabase still owns durable queue truth in the active MVP contract.
