...
- Current runtime note: the next active queue chapter is now an explicit Oracle-only cutover. Queue-ledger `primary` no longer treats Supabase queue participation as the intended end state; `ORACLE_QUEUE_SUPABASE_COMPAT_ENABLED=true` is the temporary rollback lever while Oracle-only queue burns in.
