...
- Current runtime follow-up now also treats source-item-ledger `primary` empty Oracle source-item results as authoritative on the normal path, skips no-op Supabase compatibility updates for unchanged rows, and logs the remaining compatibility rereads as `source_item_fallback_read`.
