...
- Follow-up blueprint-like ownership work now also routes Wall and Home `For You` liked-state enrichment through the injected Oracle-backed like-state reader/API, so normal feed hydration no longer rereads Supabase `blueprint_likes`.
...
- Current transcript fetch starts with opportunistic `youtube_timedtext` behind the existing provider seam and falls through to `videotranscriber_temp`, then `transcriptapi`, when YouTube captions are unavailable.
- Expected native-caption misses from `youtube_timedtext` (`NO_CAPTIONS`, `TRANSCRIPT_EMPTY`) are treated as neutral fallback events, not provider-circuit failures; rate limits, timeouts, fetch failures, and access/provider errors remain circuit-relevant.
...
- Active follow-up latency work now also lets high-priority manual generation clear its own Oracle queue sweep/claim cooldowns before waking the worker, so interactive unlock/search/manual-refresh jobs do not wait behind empty-sweep backoff meant for background queue control.
...
- Current runtime follow-up now also uses a tighter Oracle-primary cadence for fresh creator uploads: live Oracle runs `ORACLE_SUBSCRIPTION_PRIMARY_MIN_TRIGGER_INTERVAL_MS=300000` and `ORACLE_SUBSCRIPTION_REVISIT_ACTIVE_MS=300000`, while normal/quiet revisit windows remain conservative.
...
- Current runtime follow-up now also treats source-item-ledger `primary` empty Oracle source-item results as authoritative on the normal path, skips no-op Supabase compatibility updates for unchanged rows, keeps `updated_at`-only source-item churn Oracle-local instead of mirroring it back into Supabase, updates compatibility rows by durable `id` before any canonical-key conflict fallback, reuses one mapped payload helper across the shared update/insert shadow seam, and logs the remaining compatibility rereads as `source_item_fallback_read`.
- Current runtime follow-up now also supports Oracle durable generation execution truth behind `ORACLE_GENERATION_STATE_MODE=supabase|dual|primary`, so `source_item_blueprint_variants` plus `generation_runs` can bootstrap into a local Oracle ledger, dual-write in `dual`, and in `primary` let shared variant ownership/readiness plus generation-run summaries prefer Oracle-backed execution state while the later generation-trace chapter severs `generation_run_events`.
- Current runtime follow-up now also removes Supabase generation-state rehydration from Oracle bootstrap in `primary`, so restart/bootstrap keeps execution truth from Oracle `generation_variant_state` and `generation_run_state`, and startup count reporting comes from those Oracle-owned tables instead of rereading Supabase generation-state rows.
- Current runtime follow-up now also lands Oracle-only generation trace event ownership in `primary`: shared milestone/terminal event appends plus trace-detail event pagination resolve Oracle event state, with ordinary runtime `generation_run_events` rereads removed from the hot path.
- Current runtime follow-up now also completes Oracle provider-circuit-state ownership in `primary`: normal success/failure/open/close mutations, provider-availability checks, and ops snapshots now all resolve through local `provider_circuit_state` instead of ordinary Supabase runtime reads/writes.
- Current runtime follow-up now also removes normal-runtime Supabase generation-state shadow writes in `primary`, so variant claim/ready/failed transitions plus generation-run lifecycle writes stay Oracle-only and Supabase generation-state no longer matters for normal mutation correctness.
- Current runtime follow-up now also removes the main normal-runtime Supabase generation-state read fallbacks in `primary`, so shared variant/run readers, availability checks, and profile/detail generation attribution now treat Oracle execution state as authoritative instead of rereading Supabase on ordinary runtime misses.
- Current runtime follow-up now also splits manual creator lookup into explicit `Handle`, `Creator name`, and `Channel URL / ID` modes, with handle mode preferring official YouTube `forHandle` resolution before any HTML fallback.
- Current runtime follow-up now also hardens that `dual` stage by preserving Oracle variant ids on future Supabase shadow creates and comparing parity by logical variant key plus normalized run payloads, so drift checks stay focused on real execution-state mismatches.
...
- Current runtime follow-up now also treats source-item `primary` as a clean reader cutover: by-id and by-video source-item lookups should resolve from the durable Oracle source-item ledger first, while the older product-source-item mirror remains compatibility/bootstrap state instead of a normal read dependency.
...
- Current runtime follow-up now also stages Oracle durable feed truth behind `ORACLE_FEED_LEDGER_MODE=supabase|dual|primary`, so `user_feed_items` can bootstrap into a local Oracle ledger, dual-write in `dual`, and in `primary` let wall/profile/public feed reads plus shared feed mutation paths prefer Oracle-backed feed rows while Supabase becomes migration residue instead of the normal write target.
- Current runtime follow-up now also removes Supabase feed rehydration from Oracle bootstrap in `primary`, so restart/bootstrap keeps feed truth from Oracle `feed_ledger_state`, Oracle product-feed bootstrap sources feed rows from that ledger instead of rereading Supabase `user_feed_items`, and the shared Oracle-primary feed insert/upsert path no longer performs normal-runtime Supabase feed writes.
- Current runtime follow-up now also removes the main Oracle-primary `user_feed_items` read fallbacks, so wall/public/my-feed/product readers treat Oracle feed ledger + Oracle product-feed mirror as the normal runtime read path and browser-side existing-feed-item checks prefer the backend-shaped `/api/my-feed` payload before any legacy Supabase fallback.
- Current runtime follow-up now also removes source-video feed-row existence-check fallback in feed-ledger `primary`, so an Oracle miss for `user_id + source_item_id` is authoritative and normal generation/subscription flows no longer reread Supabase `user_feed_items` for that decision.
- Current runtime follow-up now also prefers earlier Home `For You` arrival for freshly discovered subscription uploads: reusable ready blueprints may bind immediately, and otherwise the unlockable feed row can be inserted before slower auto-unlock completion upgrades that same row.
- Current runtime follow-up now also gives Home wall ordering a deliberate dual-clock model: locked/unlockable feed rows keep their original `created_at`, while the first locked -> generated blueprint promotion stamps `generated_at_on_wall` so the blueprint resurfaces once when generation completes without re-bubbling on later updates.
- Current runtime follow-up now also keeps that Home wall ordering mixed correctly across card kinds: generated rows sort by `generated_at_on_wall || created_at`, while locked rows sort by `created_at`, so older generated rows no longer stay above newer locked rows by default.
- Current runtime follow-up now also applies that effective wall timestamp inside Oracle feed-ledger/product-feed pre-limit ordering, so users with many generated cards still get older locked rows into the Home `For You` candidate set.
...
- Current runtime follow-up now also gives that stale-subscription recovery path one bounded creator-title fallback after URL recovery fails on `404`, but only repairs the subscription when creator-name lookup yields one strong canonical winner.
...
- Current YT2BP one-step prompt default is `docs/golden_blueprint/golden_bp_prompt_contract_one_step_v6.md`; the runtime schema is still `blueprint_sections_v1`, with `Caveats` written into the existing `open_questions` field while `Takeaways` stay lighter plain-English skim value, `Storyline` stays `2-3` substantial paragraphs/slides, long transcript pruning is treated as normal runtime shaping rather than a caveat trigger, and the caveat philosophy now prefers balancing nuance over repetitive evidence-policing.
...
- Current runtime follow-up also introduced explicit queue fallback attribution so the egress-reduction chapter could measure any remaining Supabase queue traffic before the later Oracle-only cutover removed it from normal runtime.
- Current runtime follow-up also skipped the Supabase `ingestion_jobs` compatibility upsert for queue claim-to-`running` transitions in queue-ledger `primary`, and expanded that intermediate fallback attribution across latest-for-user, active-for-user, refresh-pending dedupe, unlock-job lookup, and retry-dedupe queue reads before those runtime fallbacks were retired.
- Current runtime follow-up now also lets Oracle-primary retry requeues stay Oracle-local when a claimed job is rescheduled back to `queued`, so Supabase `ingestion_jobs` is no longer patched just to mirror the retry-state transition.
- Current runtime note at that stage: the next active queue chapter was the Oracle-only cutover. That later completed, and queue-ledger `primary` now runs as Oracle-only normal runtime without the old Supabase queue rollback lever.
- Later Oracle-only queue closure also forced the unlock compatibility rule to tighten: Supabase `source_item_unlocks` rows can no longer mirror queue `job_id` values once queue ownership leaves Supabase, because the old `job_id` foreign key still points at Supabase `ingestion_jobs`.
...
- Current runtime note: later unlock-ownership follow-up also stopped rehydrating Oracle unlock state from Supabase shadow during bootstrap, and product unlock mirror rebuilds should now source unlock rows from the Oracle unlock ledger instead of `source_item_unlocks`.
- Current runtime note: later unlock-ownership follow-up also removed normal-runtime Supabase unlock dependence in the main server path, so reservation/processing/fail/ready mutations, transcript suppression/revalidate seed reads, and legacy `GET /api/my-feed` unlock hydration now stay on Oracle-owned unlock state in unlock-ledger `primary`.
...
- Step 25 subscription auto-unlock v1 adds per-subscription `auto_unlock_enabled` (default `true`) and bounded `source_auto_unlock_retry` attempts for transient blockers; funded-subscriber shared-cost billing is now active through canonical auto intents and participant snapshots. Current runtime treats `NO_ELIGIBLE_USERS` as terminal/no-op for retry scheduling so locked cards stay manual-unlockable without recurring retry churn.
...
- Step 26a queue-health operator semantics now clarify that `worker_running` means fresh running-job activity, not service liveness; additive fields expose activity state/reason and the liveness source operators should use in split-worker production.
...
- Step 29b queued interactive generation now also supports in-flight refill when later manual jobs arrive during an already-running interactive job, reducing burst-start lag without broadly accelerating background scopes.
