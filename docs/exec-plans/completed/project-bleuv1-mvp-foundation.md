...
- Current runtime follow-up now also stages Oracle product-state mirroring behind `ORACLE_PRODUCT_MIRROR_*`, so active subscriptions, recent source items, source-item unlock rows, and recent feed rows can be mirrored into local SQLite for Oracle-first source-page access and unlock-status checks, while Supabase still remains the authoritative product ledger and wall/profile hydration stays on the conservative Supabase-first path.
