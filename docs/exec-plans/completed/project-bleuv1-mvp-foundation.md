...
- Current runtime follow-up now also stages Oracle queue-ledger ownership behind `ORACLE_QUEUE_LEDGER_MODE=supabase|dual|primary`, so Oracle can bootstrap and shadow a local durable queue ledger before any later `primary` worker-control cutover moves claim/lease/fail/finalize authority off Supabase.
