...
- Active follow-up egress work now also uses a medium-impact cadence pass instead of aggressive disablement: combined worker maintenance is time-gated (`15m` default), `all_active_subscriptions` prioritizes the stalest `last_polled_at` rows and caps each run to `75` subscriptions by default, and passive `latest-mine` restore churn is reduced with a tighter recent-row read plus slower shared tracker polling.
- Current runtime now also includes the first Oracle control-plane bootstrap/shadow foundation behind `ORACLE_CONTROL_PLANE_*` env flags: local SQLite scheduler state can bootstrap active YouTube subscriptions and record shadow scheduler decisions for future migration, while Supabase still owns durable queue truth in the active MVP contract.
