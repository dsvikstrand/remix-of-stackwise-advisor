...
> Replacement active program: `docs/exec-plans/active/bleuv1-mvp-hardening-playbook.md`.
...
- D-024: Project 3 Step 3 telemetry lock: `p3_step3_v0` client events stored in `mvp_events` with per-session `session_id`, bucketed join failures, and a Supabase-backed weekly metrics script (`npm run metrics:channels`) with GO/HOLD/PIVOT thresholds.
- D-025: Project 3 Step 3 pilot started (2026-02-13): metrics pull validated against live Supabase events. Note: `time_to_first_join_sec` can be inflated by idle tabs because `t0` is earliest Step 3 event in a session.
- D-026: Channel-scoped posting lock (MVP): `+ Create` uses a channel-first flow (where to post -> source), auto-joins the selected curated channel, and blocks public publishing unless a valid channel context is present and joined. Library/YouTube are removed from top nav and are reached via create flow and deep links.
- D-027: App-wide wall-to-wall layout migration (Run 1): remove ambient background blobs/gradients, standardize page width (`max-w-3xl`) and tight gutters (`px-3 sm:px-4`), and retire heavy card chrome on major surfaces. Goal is consistent Reddit-style rhythm across the app without changing runtime behavior.
- D-028: App-wide wall-to-wall layout migration (Run 2): convert remaining detail pages to document-like minimal sections, retune global primitives (Card + header/nav + button variants) to remove frosted/blur defaults, flatten remaining Home modules, and fix `/tags` link debt by routing tag clicks to Explore deep links (`/explore?q=...`).
