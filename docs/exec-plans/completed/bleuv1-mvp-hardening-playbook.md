...
a4xteab) [have] Source-item-ledger `primary` now also treats empty Oracle source-item results as authoritative on the normal path, skips no-op Supabase compatibility updates for unchanged rows, and logs the remaining compatibility rereads as `source_item_fallback_read`.
