...
- Source-item ownership Pass 3 now removes the main normal-runtime Supabase `source_items` reads in `primary`: `/api/my-feed` source hydration, feed accept/source lookup helpers, and browser-side blueprint/profile source attribution now resolve through Oracle-aware server readers instead of direct Supabase `source_items` reads.
...
- Queued manual YT2BP work now also preserves interactive request class through the worker path: search/manual/source-page generation can use tighter transcript/LLM retry caps plus additive per-stage timing logs, while background ingestion keeps the slower resilience profile.
...
- Follow-up comments ownership work now also routes normal runtime blueprint YouTube comment refresh/delete-reseed writes and backend comment reads through Oracle comment state, pushing Supabase `blueprint_youtube_comments` toward compatibility-only status.
- Follow-up blueprint-tag ownership work now also routes the main backend blueprint-tag writes through Oracle `blueprint_tag_state` for blueprint creation and channel publish flows. Bootstrap seeds existing joins there, and current runtime tag reads for wall/channel/source-page/search-oriented surfaces now resolve through Oracle-backed rows instead of direct Supabase `blueprint_tags`.
- Follow-up blueprint-like ownership work now also routes Wall and Home `For You` liked-state enrichment through the injected Oracle-backed like-state reader/API, leaving Supabase `blueprint_likes` as bootstrap/break-glass residue rather than a normal feed-read fallback.
- Signed-in logo clicks now act as a Home reset on `/wall`: Wall query/filter state clears, the default lane resolves again, and the current page scrolls to the top.
...
a4n) [have] `youtube_timedtext` is now the current default transcript path; `videotranscriber_temp` is the built-in second fallback and `transcriptapi` is the built-in third fallback when YouTube captions are unavailable.
a4n0) [have] Expected native-caption misses from `youtube_timedtext` (`NO_CAPTIONS`, `TRANSCRIPT_EMPTY`) now fall through without provider-circuit failure bookkeeping; real timedtext provider trouble still contributes to circuit degradation state.
a4o) [have] Subscribe/reactivate now also backfills sparse Home `For You` walls (`<20` visible cards) with up to the latest `5` creator videos, but historical backlog stays locked unless the backend can already reuse a ready blueprint for that source item.
...
a4xu) [have] High-priority manual generation now also expedites its own Oracle queue sweep/claim cooldowns before waking the worker, so interactive unlock/search/manual-refresh jobs do not inherit multi-minute empty-sweep backoff from idle queue periods.
...
a4xef) [have] Current runtime discovery tuning now also uses a tighter Oracle-primary cadence for fresh creator uploads: the live Oracle rollout runs `ORACLE_SUBSCRIPTION_PRIMARY_MIN_TRIGGER_INTERVAL_MS=300000` and `ORACLE_SUBSCRIPTION_REVISIT_ACTIVE_MS=300000`, while normal/quiet revisit windows stay wider.
...
a4xu2) [have] Current runtime now also treats empty Oracle queue-ledger reads as authoritative in `primary` for owner/scope/latest queue lookups, avoiding fallback Supabase rereads when Oracle simply has no matching queued/running row.
a4xu3) [have] Queue lease-heartbeat-only refreshes now also skip the full Supabase `ingestion_jobs` shadow upsert in queue-ledger `primary`, trimming control-plane compatibility churn without changing visible queue behavior.
a4xu4) [have] That intermediate queue phase also introduced explicit queue fallback attribution so the egress-reduction chapter could measure any remaining Supabase queue reads before the later Oracle-only cutover removed them from normal runtime.
a4xu5) [have] Queue claim-to-`running` transitions also skipped the Supabase `ingestion_jobs` compatibility upsert in queue-ledger `primary`, and the same intermediate phase extended queue-fallback attribution across latest-for-user, active-for-user, refresh-pending dedupe, unlock-job lookup, and retry-dedupe queue reads before those normal-runtime fallbacks were retired.
a4xu6) [have] Oracle-primary retry requeues may now also stay Oracle-local when a claimed job is rescheduled back to `queued`, so Supabase `ingestion_jobs` is no longer patched just to mirror that retry-state transition.
a4xu7) [have] Queue ownership later completed the Oracle-only cutover chapter: queue-ledger `primary` now runs as Oracle-only normal runtime and the temporary Supabase queue rollback lever is no longer part of steady-state runtime.
a4xu7a) [have] That later Oracle-only queue closure also required unlock compatibility rows to stop mirroring queue `job_id` into Supabase `source_item_unlocks`, because those rows still foreign-key to Supabase `ingestion_jobs` even after queue ownership moved to Oracle.
...
a4xta1) [have] Subscription-ledger `primary` reads now also treat empty Oracle results as authoritative for hot subscription lookups (`by id`, `by user + channel`, source-page access, per-user lists/counts), avoiding fallback Supabase rereads when Oracle simply has no matching subscription row.
a4xta2) [have] Subscription compatibility shadow writes now also skip no-op `user_source_subscriptions` patches when the meaningful persisted fields are already unchanged, trimming backend PATCH churn without changing visible subscription behavior.
a4xta2a) [have] Current runtime now also keeps recurring Oracle-primary sync/checkpoint/error-only subscription patches off the Supabase compatibility row when only operational fields changed, so `last_polled_at` / checkpoint / sync-error churn no longer forces a shadow `PATCH` every time.
a4xta3) [have] Any remaining Supabase subscription reads in that mode now also log `subscription_fallback_read`, making the subscription-egress reduction chapter measurable without changing subscription correctness semantics.
...
a4xtd1) [have] Later unlock-ownership follow-up also removed Supabase unlock shadow as a bootstrap input in `primary`: Oracle restart/bootstrap should keep unlock truth from the Oracle unlock ledger itself, and product unlock mirror rebuilds should source unlock rows from Oracle instead of re-importing stale `source_item_unlocks`.
a4xtd2) [have] Later unlock-ownership follow-up also removed normal-runtime Supabase unlock dependence in the main server path: unlock reservation/processing/fail/ready mutations, transcript suppression/revalidate seed reads, and legacy `GET /api/my-feed` unlock hydration now stay on Oracle-owned unlock state in `primary`.
a4xtd3) [have] Later unlock-ownership closure also removed the remaining meaningful active-runtime product/browser reads of `source_item_unlocks`, so Blueprint Detail source attribution, liked-blueprint profile linkage, and the browser-side legacy `My Feed` fallback no longer depend on direct Supabase unlock-row queries.
a4xte) [have] Current runtime now also stages Oracle durable feed truth behind `ORACLE_FEED_LEDGER_MODE=supabase|dual|primary`, so `user_feed_items` can bootstrap into a local Oracle ledger, dual-write in `dual`, and in `primary` let wall/profile/public feed reads plus shared feed mutation paths prefer that Oracle-backed feed row store while Supabase becomes migration residue instead of the normal write target.
a4xte0) [have] Later feed-ownership follow-up also removes Supabase feed rehydration from Oracle bootstrap in `primary`: Oracle restart/bootstrap keeps durable feed truth from `feed_ledger_state`, Oracle product-feed bootstrap sources feed rows from that Oracle ledger instead of rereading Supabase `user_feed_items`, and the shared Oracle-primary feed insert/upsert path no longer performs normal-runtime Supabase feed writes.
a4xte0a) [have] Feed-ownership Pass 3 later removes the main Oracle-primary `user_feed_items` read fallbacks too: wall/public/my-feed/product readers now treat Oracle feed ledger + Oracle product-feed mirror as the normal runtime read path, and browser-side existing-feed-item checks prefer the backend-shaped `/api/my-feed` payload before any legacy Supabase fallback.
a4xte0b) [have] Feed-ownership follow-up later makes per-user source-video feed-row existence checks Oracle-authoritative in `primary`, so an empty `feed_ledger_state` result no longer rereads Supabase `user_feed_items` before subscription/search/manual/source-page generation decides insert vs skip.
a4xte1) [have] Current runtime now also prefers earlier Home `For You` arrival for newly discovered subscription uploads: reusable ready blueprints can attach immediately, and otherwise the unlockable row may be inserted before slower auto-unlock work completes and upgrades the same feed item.
a4xte1a) [have] Feed-row wall ordering now uses dual clocks: locked/unlockable Home rows keep their original `created_at`, and the first locked -> generated blueprint promotion stamps `generated_at_on_wall` so the usable blueprint resurfaces once at generation completion time without letting later updates re-bubble the card again.
a4xte1b) [have] Mixed Home ordering now also follows one effective wall timestamp across card kinds, so generated rows no longer pin above newer locked rows solely because they have a non-null `generated_at_on_wall`.
a4xte1c) [have] Oracle feed-ledger and product-feed readers now also apply that effective timestamp before pre-limit windows, preventing generated-heavy Home `For You` accounts from dropping older locked cards out of the fetched candidate set.
...
a4xteab) [have] Source-item-ledger `primary` now also treats empty Oracle source-item results as authoritative on the normal path, skips no-op Supabase compatibility updates for unchanged rows, keeps `updated_at`-only source-item churn Oracle-local instead of mirroring it back into Supabase, updates compatibility rows by durable `id` before any canonical-key conflict fallback, reuses one mapped payload helper across the shared update/insert shadow seam, and logs the remaining compatibility rereads as `source_item_fallback_read`.
a4xteaa) [have] Current runtime now also supports Oracle durable generation execution truth behind `ORACLE_GENERATION_STATE_MODE=supabase|dual|primary`, so `source_item_blueprint_variants` and `generation_runs` can bootstrap into a local Oracle ledger, dual-write in `dual`, and in `primary` let variant ownership/readiness plus generation-run summaries prefer Oracle-backed execution state while the later generation-trace chapter severs `generation_run_events`.
a4xteaab) [have] Generation-state Pass 1 now also removes Supabase generation-state rehydration from Oracle bootstrap in `primary`: restart/bootstrap should keep execution truth from Oracle `generation_variant_state` and `generation_run_state`, and startup count reporting should read those Oracle-owned tables instead of rereading Supabase generation-state rows.
a4xteaac) [have] Generation-trace Pass 2 now also removes normal-runtime Supabase `generation_run_events` writes in `primary`, and Pass 3 now also removes the main normal-runtime trace-event reads there: shared event append/pagination plus trace detail routes now resolve Oracle event state without ordinary Supabase rereads.
a4xteaac) [have] Generation-state Pass 2 now also removes normal-runtime Supabase generation-state shadow writes in `primary`: variant claim/ready/failed transitions plus generation-run lifecycle writes now stay Oracle-only, with Supabase generation-state no longer needed for normal mutation correctness.
a4xteaad) [have] Generation-state Pass 3 now also removes the main normal-runtime Supabase generation-state read fallbacks in `primary`: shared variant/run readers plus profile/detail availability surfaces now treat Oracle execution state as authoritative, and browser-facing blueprint/source attribution resolves through backend Oracle-aware lookup instead of direct `source_item_blueprint_variants` reads.
a4xteabx) [have] Creator lookup now also supports explicit search modes (`Handle`, `Creator name`, `Channel URL / ID`), and explicit handle mode prefers official YouTube `forHandle` resolution before legacy handle-page scraping fallback.
a4xteb) [have] Source-item rollout safety now also requires Oracle-first source-item reader failures to degrade to Supabase-backed source rows during `supabase|dual`, preventing wall/source-page/unlock traffic from failing while the durable source-item ledger is still soaking.
a4xtec) [have] Once source-item runtime is in `primary`, the durable Oracle source-item ledger is now the normal Oracle-side reader for by-id and by-video source-item lookups; the older product-source-item mirror remains compatibility/bootstrap state rather than a steady-state read dependency.
...
a4xtec) [have] Browser-called Supabase edge functions now also require explicit app-origin CORS allowlists (`https://bleup.app`, `https://www.bleup.app`, local dev, and intentional preview origins) so telemetry/upload helpers do not default browser traffic back to the older GitHub preview origin.
...
a4xv) [have] Current runtime now also hardens YouTube subscription feed fetches on the Oracle-cron path: transient feed `5xx/network` failures retry inside one sync, stale `404` channel ids can recover from stored channel URLs, Oracle scheduler backoff now distinguishes transient feed failure from persistent not-found and progressively quiets repeated `404` rows (`1x` -> `2x` -> `4x` -> `8x`, capped at `24h`), and `all_active_subscriptions` no longer marks the whole batch failed unless hard failures remain or every attempted creator soft-fails.
a4xv1) [have] The same stale-subscription path now also allows one bounded creator-title fallback after URL recovery fails on `404`, but only rewrites the subscription when creator-name lookup yields one strong canonical channel winner.
a4xw) [have] Current runtime now also keeps subscription cron hard failures readable: `last_sync_error` and `PARTIAL_FAILURE` terminal summaries preserve compact `message/code/details/hint` text and emit structured `subscription_sync_hard_failed` logs instead of collapsing object-shaped errors to `[object Object]`.
a4xx) [have] Current runtime now also requires fresh `source_item_unlocks` rows plus Oracle-primary/shared unlock row builders to initialize `transcript_status='unknown'`; subscription sync inserts that leave transcript truth null are treated as live-breaking because they stop `all_active_subscriptions` from creating new locked rows.
...
a4yb) [have] Active provider-circuit migration now also keeps Oracle-owned circuit reads and writes in `primary`: provider success/failure/open/close mutations, fail-fast availability checks, and ops snapshots all resolve through local `provider_circuit_state`.
...
a4za) [have] Current YT2BP one-step prompt default is `docs/golden_blueprint/golden_bp_prompt_contract_one_step_v6.md`, keeping the same `blueprint_sections_v1` runtime shape while writing `Caveats` into the existing `open_questions` field, shifting `Takeaways` toward lighter plain-English skim value, requiring more substantial `Storyline` paragraphs/slides, treating long transcript pruning as normal runtime shaping rather than a caveat trigger, and preferring balancing nuance over repetitive evidence-policing in caveats.
...
k6) [have] Post-completion ownership maintenance on `2026-05-07`: browser tag reads for current product surfaces now resolve through backend Oracle-aware tag APIs rather than direct Supabase `tags`, `blueprint_tags`, or `tag_follows` reads; remaining tag-table Supabase access is compatibility/bootstrap/write-shadow residue tracked in the active Oracle/Supabase ownership closure program.
k7) [have] Post-completion ownership maintenance on `2026-05-08`: backend tag creation, tag follow/unfollow, and backend blueprint-tag assignment now use Oracle `tag_state`, `tag_follow_state`, and `blueprint_tag_state` as the normal owner; remaining Supabase tag-family access is legacy fallback/bootstrap/browser-create residue tracked in the active Oracle/Supabase ownership closure program.
k8) [have] Post-completion ownership maintenance on `2026-05-10`: current browser blueprint create/update/save flows call backend `/api/blueprints*` routes instead of writing Supabase `blueprints` / `blueprint_tags` directly; the backend keeps required compatibility rows while Oracle blueprint/tag state carries the active owner path.
k9) [have] Post-completion ownership maintenance on `2026-05-10`: current browser blueprint list/search/count surfaces now call backend `/api/blueprints` or blueprint detail routes backed by Oracle `blueprint_state`; direct browser Supabase `blueprints` reads are no longer expected for active product surfaces.
k10) [have] Post-completion ownership maintenance on `2026-05-10`: backend tag-family fallback now fails closed by default; normal runtime tag creation and blueprint-tag attachment require Oracle-aware owner dependencies, and Supabase fallback needs explicit break-glass env plus logs.
