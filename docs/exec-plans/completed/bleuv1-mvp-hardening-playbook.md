...
a4xte0) [have] Later feed-ownership follow-up also removes Supabase feed rehydration from Oracle bootstrap in `primary`: Oracle restart/bootstrap should keep durable feed truth from `feed_ledger_state`, and Oracle product-feed bootstrap should source feed rows from that Oracle ledger instead of rereading Supabase `user_feed_items`.
