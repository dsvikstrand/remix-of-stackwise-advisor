...
a4xc) [have] Current runtime groundwork now also includes an Oracle control-plane subscription-scheduler path (`ORACLE_CONTROL_PLANE_*`) that can initialize local SQLite scheduler state, bootstrap active YouTube subscriptions, persist shadow scheduler decisions, and in `primary` own `all_active_subscriptions` enqueue admission plus batch selection, while Supabase remains the durable queue truth in MVP.
