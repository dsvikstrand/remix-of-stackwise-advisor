...
a4xx) [have] Current runtime now also requires fresh `source_item_unlocks` rows to initialize `transcript_status='unknown'`; subscription sync inserts that leave transcript truth null are treated as live-breaking because they stop `all_active_subscriptions` from creating new locked rows.
