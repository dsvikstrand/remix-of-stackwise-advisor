...
a4yb) [have] Active provider-circuit migration now also starts Oracle-owned circuit writes in `primary`: provider success/failure/open/close mutations persist local `provider_circuit_state`, while availability reads and ops snapshots stay on the previous reader path until the follow-up pass.
