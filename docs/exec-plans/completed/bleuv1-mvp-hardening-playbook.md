...
a4xc) [have] Current runtime groundwork now also includes a default-off Oracle control-plane bootstrap path (`ORACLE_CONTROL_PLANE_*`) that can initialize local SQLite scheduler state for future subscription-scheduler migration, while Supabase remains the durable queue truth in MVP.
