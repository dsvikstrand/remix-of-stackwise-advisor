...
a4t) [have] Active Supabase egress-reduction work now skips unchanged successful subscription sync writes (`user_source_subscriptions`) unless checkpoint/title/error state changes, while repeated identical error writes remain bounded by a `30m` backend heartbeat and `all_active_subscriptions` enqueue is gated to an effective `60m` minimum interval; treat that as current runtime truth even though detailed proof/next steps live in the active egress plan.
...
a4xc) [have] Current runtime groundwork now also includes an Oracle control-plane bootstrap/shadow path (`ORACLE_CONTROL_PLANE_*`) that can initialize local SQLite scheduler state, bootstrap active YouTube subscriptions, and persist shadow scheduler decisions for future subscription-scheduler migration, while Supabase remains the durable queue truth in MVP.
