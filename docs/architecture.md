...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle queue-control can now also gate repeated empty claim attempts in the queued worker using Oracle-local cooldown state (`ORACLE_QUEUE_*` envs), reducing claim-loop chatter without moving durable queue rows, claim truth, or lease truth off Supabase.
