...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle durable feed truth is now also staged behind `ORACLE_FEED_LEDGER_MODE=supabase|dual|primary`: Oracle can bootstrap a local SQLite ledger for `user_feed_items`, shadow feed-row insert/update/delete transitions in `dual`, and then let wall/profile/public feed readers prefer that durable feed ledger while Supabase remains the compatibility shadow underneath.
