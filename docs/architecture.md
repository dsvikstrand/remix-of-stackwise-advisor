...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle queue-admission and job-activity mirrors can now also rebuild from that live Oracle queue ledger first, so their own refresh/bootstrap paths stop depending on Supabase when the ledger is already authoritative.
