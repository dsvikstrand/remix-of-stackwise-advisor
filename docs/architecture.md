...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle product-state mirroring is now available behind `ORACLE_PRODUCT_MIRROR_*` envs: Oracle can bootstrap/mirror active subscriptions, recent source items, source-item unlock rows, and recent feed rows into local SQLite so source-page subscription/access checks, unlock-status lookups, and wall/profile feed-history hydration can prefer Oracle-local reads first when mirror coverage is sufficiently complete. Hot subscription/feed/unlock mutations now also refresh the Oracle product mirror from known rows or targeted reloads so those readers stay aligned, while Supabase remains the authoritative product ledger and fallback source underneath.
