...
- Frontend:
  ...
  - Backend:
    ...
    - Main backend blueprint-tag ownership now also uses Oracle `blueprint_tag_state`: blueprint creation, auto-channel publish, and manual channel publish write tag joins there, while coupled backend tag reads (classification, channel feed/evaluate, auto-banner) prefer Oracle rows and only fall back per-blueprint to Supabase for pre-cutover residue.
...
- Data:
  ...
  - ingestion worker hardening:
    ...
    - Blueprint tags are now also staged on Oracle-owned backend state for the main runtime path: creation/publish writes stay on `blueprint_tag_state`, while coupled backend tag reads prefer Oracle and only fall back per-blueprint for older Supabase residue.
