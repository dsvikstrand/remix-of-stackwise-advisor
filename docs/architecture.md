...
- Data:
  ...
  - ingestion worker hardening:
    ...
    - YouTube refresh bookkeeping now skips unchanged `source_items.metadata.view_count` writes, unchanged `blueprint_youtube_comments` snapshot rewrites, and no-op `blueprint_youtube_refresh_state` upserts, reducing refresh-state churn without changing refresh UX.
