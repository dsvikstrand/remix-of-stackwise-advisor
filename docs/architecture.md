...
- Frontend:
  ...
  - Backend:
    ...
    - Generation-trace Pass 2 now removes normal-runtime Supabase `generation_run_events` writes in `primary`: event sequencing and append writes land in local Oracle control-plane state, while trace detail reads remain the next severing pass.
...
- Data:
  ...
  - durable generation trace foundation: `generation_runs` (run summary/model/quality terminal state) + `generation_run_events` (ordered stage-level events); 30-day event retention via service-role purge function, run summary retained indefinitely.
    ...
    - Oracle generation-state migration covers `generation_runs` summaries and `source_item_blueprint_variants` first; the follow-up generation-trace chapter now also moves normal runtime `generation_run_events` writes onto Oracle control-plane state, with Oracle-only trace reads still queued as the remaining severing pass.
