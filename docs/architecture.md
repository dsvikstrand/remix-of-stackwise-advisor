...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle durable source-item truth is now also staged behind `ORACLE_SOURCE_ITEM_LEDGER_MODE=supabase|dual|primary`: Oracle can bootstrap a local SQLite ledger for `source_items`, shadow source-item upserts plus source-item metadata/view-count updates in `dual`, and in `primary` let execution-path source lookups plus Oracle-first wall/profile/source-page source-row reads prefer that durable source-item ledger while Supabase remains the compatibility shadow underneath.
