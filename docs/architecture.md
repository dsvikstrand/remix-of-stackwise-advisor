...
- Frontend:
  ...
  - Backend:
    ...
    - Provider-circuit-state Pass 1 now keeps normal runtime provider circuit writes on Oracle control-plane state: provider success/failure/open/close transitions persist through local `provider_circuit_state`, while read-side availability checks and ops queue-health snapshots stay on the existing reader path until the follow-up read cutover.
...
- Data:
  ...
  - provider circuit control-plane state is now also on the Oracle ownership path: Pass 1 persists normal runtime circuit mutations to local `provider_circuit_state`, while the older availability/snapshot reader remains the rollback-safe read path until Pass 2.
