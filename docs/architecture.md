...
- Frontend:
  ...
  - High-priority interactive generation work is now cadence-aware in the opposite direction:
    - fresh manual `source_item_unlock_generation`, `search_video_generate`, and `manual_refresh_selection` enqueues may clear their own Oracle-local queue sweep/claim cooldowns before waking the worker
    - this preserves aggressive start latency for user-triggered work without removing background backoff protections for idle sweeps
  ...
  - Personal workspace is profile-first at `/u/:userId` with tabs `Feed / Comments / Liked`; the `Feed` tab is read-only profile history for generated blueprints and subscribed creators, while `/my-feed` is compatibility-only and redirects to `/wall`.
    - liked-blueprint source linkage for active profile surfaces now resolves through variants/feed context rather than direct Supabase `source_item_unlocks` reads.
  ...
  - Live adapter UI in `src/pages/YouTubeToBlueprint.tsx`.
    ...
    - the default one-step prompt template is `docs/golden_blueprint/golden_bp_prompt_contract_one_step_v6.md`; it keeps the same schema, uses the existing `open_questions` field for a more reader-useful `Caveats` section, retunes `Takeaways` toward lighter plain-English skim value, requires `Storyline` to stay `2-3` substantial paragraphs/slides, and treats long transcript pruning as normal runtime shaping rather than a caveat trigger.
    - deterministic transcript pruning now defaults to `5000` chars with threshold buckets `5000/9000/16000`; long transcripts still use the same evenly spaced sampling windows before the final cap is enforced.
  ...
  - Legacy compatibility surface in `src/pages/MyFeed.tsx` remains code-present, but `/my-feed` now redirects to `/wall`.
    ...
    - the older browser-side Supabase fallback is now cache-only when backend API access is unavailable or unset; it no longer reconstructs unlock rows from direct `source_item_unlocks` reads.
  - Blueprint detail in `src/pages/BlueprintDetail.tsx` now prefers source-channel attribution for imported YouTube blueprints and hides edit CTA in default MVP UI.
    - source-channel lookup for that surface now resolves through Oracle-owned/source-linked variant or feed context rather than direct Supabase `source_item_unlocks` reads.
  ...
  - Source-item ownership is now entering the same cutover pattern:
    ...
    - current source-item Pass 2 removes normal-runtime Supabase `source_items` writes in Oracle-primary mutation paths: source-item insert/upsert plus metadata/view-count updates now write Oracle source-item ledger + Oracle product-source mirror only.
    - current source-item Pass 3 removes the main normal-runtime Supabase `source_items` reads in Oracle-primary mode: `/api/my-feed` now hydrates source rows through the Oracle-aware source reader, feed-route accept/source lookup paths no longer depend on direct `source_items` reads in normal runtime, and browser-side blueprint/profile source attribution can resolve source rows through the backend Oracle-aware lookup path instead of reading Supabase `source_items` directly.
  ...
  - Notifications runtime ownership is now staged:
    - inbox reads, write-side notification persistence, and read-state mutations are Oracle-first through the control-plane store.
    - current Supabase `notifications` participation is reduced to the compatibility shadow needed for the trigger-driven push-dispatch enqueue path.
  ...
  - Backend:
    ...
    - interactive high-priority generation now also has an Oracle-local expedite path layered on top of that control state, so fresh user-triggered queue inserts can reset their own high-priority sweep/claim cooldowns before the worker is scheduled.
    ...
    - Oracle queue-ledger ownership is now staged behind `ORACLE_QUEUE_LEDGER_MODE=supabase|dual|primary`: Oracle can bootstrap a local SQLite queue ledger from the durable queue and, in `primary`, now act as the live worker-control source for enqueue/claim/lease/fail/finalize while Supabase is reduced to compatibility shadowing.
    - Queue depth/work-item/status reads can now also resolve from that Oracle queue ledger first, so admission checks, queue-health snapshots, active-scope checks, queued ordering, and several latest-job reads no longer need to prefer Supabase once the queue ledger is live.
    - Oracle queue-admission and job-activity mirrors can now also rebuild from that live Oracle queue ledger first, and in `primary` the normal hot-path reads now prefer the queue ledger directly while those mirrors are reduced to fallback/bootstrap compatibility surfaces instead of the main runtime source.
    ...
    - Remaining queue compatibility patches are now also action-class aware: the Supabase shadow path distinguishes terminal vs retry/claim/lease transitions and narrows `PATCH /ingestion_jobs` payloads to the fields needed for that lifecycle class instead of rewriting the full queue row on every remaining shadow update.
    - Oracle durable subscription truth is now also staged behind `ORACLE_SUBSCRIPTION_LEDGER_MODE=supabase|dual|primary`: Oracle can bootstrap a local SQLite ledger for `user_source_subscriptions`, shadow subscribe/unsubscribe/checkpoint/error writes in `dual`, and then serve Oracle-first subscription/access/count/list reads from that durable ledger while Supabase remains the compatibility shadow underneath.
    - Oracle-backed subscription helpers now also handle batch hydration by subscription `id` plus active-user fan-out by source page/channel, so due-batch sync selection, manual refresh checkpoint writes, shared blueprint attach, and auto-unlock eligibility no longer need direct Supabase `user_source_subscriptions` reads on the normal path.
    - Supabase subscription shadow updates now prefer a direct write by durable subscription `id` before any user/channel reread fallback, which keeps compatibility writes aligned with the Oracle-ledger row chosen in the primary runtime path while still allowing safe backfill fallback.
    - Oracle durable feed truth is now also staged behind `ORACLE_FEED_LEDGER_MODE=supabase|dual|primary`: Oracle can bootstrap a local SQLite ledger for `user_feed_items`, shadow feed-row insert/update/delete transitions in `dual`, and in `primary` let wall/profile/public feed readers plus shared feed mutation paths use that durable feed ledger while Supabase becomes migration residue instead of the normal write target.
    - Feed ownership Pass 1 removed Supabase feed rehydration from Oracle bootstrap in `primary`: Oracle restart/bootstrap keeps durable feed truth from the Oracle feed ledger itself, and Oracle product-feed mirror rebuilds source feed rows from `feed_ledger_state` instead of rereading Supabase `user_feed_items`.
    - Feed ownership Pass 2 removes the main Oracle-primary `user_feed_items` insert/upsert shadow writes; shared feed mutation paths now persist to Oracle `feed_ledger_state` + `product_feed_state`, with the remaining read-side Supabase severing handled in later feed passes.
    - Feed ownership Pass 3 removes the main Oracle-primary `user_feed_items` read fallbacks as well: wall/public/my-feed/product readers now treat Oracle feed ledger + Oracle product-feed mirror as the normal read path, and browser-side existing-feed-item checks prefer the backend-shaped Oracle-aware `/api/my-feed` payload before any legacy Supabase fallback.
    - Feed ownership follow-up also removes the source-video existence-check fallback in `primary`: backend `user_id + source_item_id` checks now treat an empty Oracle feed-ledger result as authoritative and do not reread Supabase `user_feed_items`; the product-feed mirror is only an Oracle-local secondary helper.
    - Oracle durable source-item truth is now also supported behind `ORACLE_SOURCE_ITEM_LEDGER_MODE=supabase|dual|primary`: Oracle can bootstrap a local SQLite ledger for `source_items`, shadow source-item upserts plus source-item metadata/view-count updates in `dual`, and in `primary` let execution-path source lookups plus Oracle-first wall/profile/source-page source-row reads prefer that durable source-item ledger while Supabase remains the compatibility shadow underneath. If those Oracle-first source-item reads regress, the operational rollback is to return live runtime to `supabase` until the fixed build is redeployed.
    - In source-item-ledger `primary`, empty Oracle results for hot source-item lookups are now authoritative in the normal runtime path, so by-id, by-canonical-key, by-video-id, and batch source-row hydration should not automatically reread Supabase when Oracle has no matching row.
    - Supabase source-item shadow updates now also skip no-op writes when the compatibility row already matches the meaningful persisted fields; `updated_at`-only source-item churn now stays Oracle-local instead of triggering a compatibility patch, normal Oracle-primary compatibility writes now update by durable `id` before any canonical-key conflict fallback instead of eagerly rereading Supabase `source_items` by `id` + `canonical_key`, that shared update/insert seam now also reuses one mapped payload helper on both paths, and any remaining source-item rereads in that mode should log `source_item_fallback_read` for attribution.
    - Oracle durable generation execution truth is now also supported behind `ORACLE_GENERATION_STATE_MODE=supabase|dual|primary`: Oracle can bootstrap local SQLite state for `source_item_blueprint_variants` and `generation_runs`, shadow variant/run writes in `dual`, and in `primary` let shared generation-state readers/writers prefer Oracle-backed variant ownership plus run-summary rows while Supabase remains the compatibility shadow underneath.
    - Generation-state Pass 1 now removes Supabase generation-state rehydration from Oracle bootstrap in `primary`: restart/bootstrap keeps execution truth from `generation_variant_state` and `generation_run_state`, and startup count reporting now reads those Oracle-owned tables instead of rereading Supabase `source_item_blueprint_variants` or `generation_runs`.
    - Generation-state Pass 2 now removes the main normal-runtime Supabase generation-state shadow writes in `primary`: Oracle variant claim/ready/failed transitions plus generation-run lifecycle writes now stay on `generation_variant_state` and `generation_run_state`, with Supabase retained only for later read-side cleanup and historical residue.
    - Generation-state Pass 3 now removes the main normal-runtime Supabase generation-state read fallbacks in `primary`: shared variant/run readers, blueprint-availability cooldown checks, and profile/detail generation attribution now treat `generation_variant_state` and `generation_run_state` as the normal runtime read source, while direct browser `source_item_blueprint_variants` reads are replaced with backend Oracle-aware lookups.
    - Generation-trace Pass 2 now removes normal-runtime Supabase `generation_run_events` writes in `primary`: event sequencing and append writes land in local Oracle control-plane state.
    - Generation-trace Pass 3 now removes the main normal-runtime Supabase trace-event reads in `primary`: shared event pagination plus the trace-detail routes now resolve Oracle control-plane event state without rereading Supabase `generation_run_events` on ordinary runtime misses.
    - Main backend blueprint-tag ownership now also uses Oracle `blueprint_tag_state`: blueprint creation, auto-channel publish, and manual channel publish write tag joins there. Oracle bootstrap seeds existing tag joins into `blueprint_tag_state`, and runtime tag reads for wall/channels/source-page/search-suggestion surfaces now resolve through Oracle-backed reads instead of direct Supabase `blueprint_tags`.
    - Browser product surfaces now route tag reads through backend Oracle-aware APIs: `src/lib/blueprintTagsApi.ts` has no Supabase fallback, `tagsApi` covers slug/id/directory/follow reads, Channels and Explore hydrate tag data through `/api/tags*` and `/api/blueprint-tags`, and `/api/my-feed` receives blueprint-tag rows through an injected Oracle-aware backend reader.
    - Backend tag writes now also use Oracle as the normal owner: tag creation mints/loads IDs from `tag_state`, follow/unfollow mutates `tag_follow_state` plus Oracle follower counts, and backend blueprint creation / auto-channel / manual channel publish attach joins after resolving tag IDs through the shared Oracle-aware helper.
    - Blueprint-like runtime state for Wall and Home `For You` now resolves through the injected Oracle-backed `blueprint_like_state` reader/API. Supabase `blueprint_likes` is retained only for bootstrap/break-glass compatibility on that surface, not normal feed liked-state hydration.
    - Provider-circuit-state ownership now keeps normal runtime provider circuit reads and writes on Oracle control-plane state: provider success/failure/open/close transitions persist through local `provider_circuit_state`, and read-side availability checks plus ops queue-health snapshots now resolve through that Oracle-backed state without ordinary Supabase rereads.
    - Generation-state `dual` parity now treats the variant logical key (`source_item_id + generation_tier`) as the canonical comparison key and normalizes empty `quality_issues` plus small shadow-write timestamp skew, so parity stays focused on real execution-state drift instead of backend-local row-id or serialization noise.
    - During `supabase|dual` rollout, Oracle-first source-item lookup failures should now degrade to Supabase-backed source rows directly instead of letting Oracle source-item read errors take down wall/profile/source-page business flows.
    - Once `ORACLE_SOURCE_ITEM_LEDGER_MODE=primary` is live, the durable source-item ledger is the normal Oracle-side read source for by-id and by-video lookups; the older product-source-item mirror remains compatibility/bootstrap state rather than the steady-state source-item reader.
    - Source-page video-library reads must also combine durable unlock-row state with queued/running `source_item_blueprint_variants` state, so a source video already being generated is surfaced back to the client as `processing` immediately after `POST /videos/unlock` rather than waiting for a later unlock-row transition.
    - `needs_generation` variant resolution is not an in-progress state: Source Page follow-up reads should only surface `processing` when the variant is truly `queued` or `running`.
    - Source-page unlock prepare failures must not masquerade as `in_progress`: if Oracle-primary unlock mutation wrappers fail, runtime should fall back to the underlying Supabase unlock mutation path, resync the Oracle unlock/product shadows from the known durable row, and only return `in_progress` when real queued/running work exists rather than an impossible still-`available` unlock row. Legacy Supabase unlock rows with `transcript_probe_meta = null` must also be normalized back to `{}` on reserve/ensure writes so fallback mutations do not trip the table's `NOT NULL` constraint.
    - Oracle-only unlock closure now also removes the remaining meaningful active-runtime product/browser reliance on `source_item_unlocks`; any leftover Supabase unlock rows should be treated as historical residue rather than a normal read source.
    - Short-transcript failures (`TRANSCRIPT_INSUFFICIENT_CONTEXT`) are now part of the same blueprint-unavailable cooldown family used for retry-safe source-video failures, and locked-card readers on Home/Profile should suppress those rows during cooldown instead of advertising them as normal unlockable work.
    - Oracle product-state mirroring is now available behind `ORACLE_PRODUCT_MIRROR_*` envs: Oracle can bootstrap/mirror active subscriptions, recent source items, source-item unlock rows, and recent feed rows into local SQLite so source-page subscription/access checks, blueprint-cooldown decisions, unlock-status lookups, public wall/source-page blueprint feeds, and wall/profile feed-history hydration can prefer Oracle-local reads first when mirror coverage is sufficiently complete. Hot subscription/feed/unlock mutations now also refresh the Oracle product mirror from known rows or targeted reloads so those readers stay aligned, while Supabase remains the authoritative product ledger and fallback source underneath.
    - Subscription feed fetch hardening now classifies YouTube RSS failures more precisely: transient `5xx/network` feed failures are retried inside one sync and then recorded as soft per-subscription outcomes for Oracle-cron batches, `404` feed failures try stale-channel recovery through the existing channel-resolution path before downgrading to a progressively quieter Oracle revisit interval for repeated misses (`1x` -> `2x` -> `4x` -> `8x`, capped at `24h`), and `all_active_subscriptions` no longer marks the whole batch failed unless hard failures remain or every attempted subscription soft-fails.
  ...
  - subscription ingestion APIs:
    ...
    - `GET /api/source-pages/:platform/:externalId/videos` follow-up reads should stay consistent with that unlock response by reflecting variant `queued|running` state as `unlock_in_progress=true`.
    - Source-item promotion should now also be gated by a live canary: wall creation must keep advancing and source-page `POST /videos/unlock` followed by `GET /videos` must preserve the same `processing` state for in-progress work.
...
- Data:
  ...
  - durable generation trace foundation: `generation_runs` (run summary/model/quality terminal state) + `generation_run_events` (ordered stage-level events); 30-day event retention via service-role purge function, run summary retained indefinitely.
    ...
    - Oracle generation-state migration covers `generation_runs` summaries and `source_item_blueprint_variants` first; the follow-up generation-trace chapter now also moves both normal runtime `generation_run_events` writes and reads onto Oracle control-plane state.
  ...
  - provider circuit control-plane state is now also on the Oracle ownership path: normal runtime circuit mutations, fail-fast reads, and ops/provider snapshots resolve through local `provider_circuit_state`, with the older Supabase row no longer part of the steady-state control path.
  ...
  - ingestion worker hardening:
    ...
    - Blueprint YouTube comments are now also staged on an Oracle-owned comment-state sink plus backend reader route: refresh/delete-reseed writes and normal runtime reads should stay on Oracle when the control-plane/backend path is active, with direct Supabase comment reads left only as compatibility fallback.
    ...
    - Tag creation and follow-state writes now stay on Oracle-owned `tag_state` / `tag_follow_state` for the backend runtime path; Supabase tag-table access remains bootstrap, compatibility fallback, or legacy-browser-create residue rather than the normal write owner.
