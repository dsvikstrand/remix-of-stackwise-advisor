...
- Frontend:
  ...
  - Backend:
    ...
    - Supabase subscription shadow updates now prefer lookup by durable subscription `id` before falling back to user/channel identity, which keeps compatibility writes aligned with the Oracle-ledger row chosen in the primary runtime path while still allowing safe backfill fallback.
