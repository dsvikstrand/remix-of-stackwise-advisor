...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle queue-control can now also gate repeated empty claim attempts in the queued worker using Oracle-local cooldown state (`ORACLE_QUEUE_*` envs), and Oracle queue-sweep control can now own tier cadence, batch sizing, and worker wake timing for queued sweeps through `ORACLE_QUEUE_SWEEP_*` envs, without moving durable queue rows, claim truth, or lease truth off Supabase.
