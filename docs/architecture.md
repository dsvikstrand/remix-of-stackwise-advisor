...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle queue-ledger ownership is now staged behind `ORACLE_QUEUE_LEDGER_MODE=supabase|dual|primary`: Oracle can bootstrap a local SQLite queue ledger from the durable queue and, in `dual`, shadow enqueue/claim/lease/fail/finalize transitions there before any later `primary` cutover makes that ledger the live worker-control source.
