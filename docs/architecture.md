...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle control-plane subscription scheduling now runs behind runtime env flags (`ORACLE_CONTROL_PLANE_ENABLED`, scheduler mode `supabase|shadow|primary`); local SQLite scheduler state bootstraps active YouTube subscriptions, can observe shadow scheduler decisions, and in `primary` now owns `all_active_subscriptions` enqueue admission, cadence timing, external-trigger ownership, and due-batch selection, including bounded multi-batch drain inside one run, while Supabase remains authoritative for durable queue truth, leases, checkpoints, and user-facing writes.
    - Oracle queue-control can now also gate repeated empty claim attempts in the queued worker using Oracle-local cooldown state (`ORACLE_QUEUE_*` envs), Oracle queue-sweep control can now own tier cadence, batch sizing, and worker wake timing for queued sweeps through `ORACLE_QUEUE_SWEEP_*` envs, and Oracle queue-admission mirror state can now serve hot queue-depth/backpressure reads through `ORACLE_QUEUE_ADMISSION_*` envs so admission/preflight paths stop re-reading the same active queue counts from Supabase on every request. Durable queue rows, claim truth, and lease truth still remain on Supabase.
