...
- Frontend:
  ...
  - Backend:
    ...
    - Supabase source-item shadow updates now also skip no-op writes when the compatibility row already matches the meaningful persisted fields, and any remaining source-item rereads in that mode should log `source_item_fallback_read` for attribution.
