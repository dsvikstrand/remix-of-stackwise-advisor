...
- Frontend:
  ...
  - Backend:
    ...
    - Oracle product-state mirroring is now available behind `ORACLE_PRODUCT_MIRROR_*` envs: Oracle can bootstrap/mirror active subscriptions, recent source items, source-item unlock rows, and recent feed rows into local SQLite so source-page subscription/access checks and unlock-status lookups can prefer Oracle-local reads first, while Supabase remains the authoritative product ledger and wall/profile hydration stays on the existing conservative Supabase-first path until mirror coverage is broader.
