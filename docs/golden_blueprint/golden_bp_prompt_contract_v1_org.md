...
## Context Vibe Filler (Guidance Template with Placeholders)

Use this as guidance text shape, not as final drop-in prompt text.

Start by declaring audience and intent in plain language: "We are writing for users who engage with practical, high-signal content and reward clarity with comments." Then insert a short reference pointer placeholder like `<POSITIVE_REFERENCE_SET_DESCRIPTION>` and explicitly state that references are for tone and flow only.

Follow with hard guardrails in prose: "Content truth must come only from `<SOURCE_TRANSCRIPT_CONTEXT>`. Do not import facts, numbers, examples, or wording from references. If style pressure conflicts with source fidelity, source fidelity wins." End with outcome framing: "The goal is writing that makes the same audience want to read, save, and discuss this blueprint because it is useful and specific."
