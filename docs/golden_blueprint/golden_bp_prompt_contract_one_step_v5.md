# Golden BP Prompt Contract v5

Version: `v5`
Status: Canonical (current)
Owner intent: Generate high-value blueprints with a stable Bluep quality stamp, lighter fast-skim takeaways, a stricter storyline substance floor, and a more useful Caveats section.
...
Caveats highlights where the source’s reasoning, evidence, scope, or confidence gets weaker. It should provide three to five complete bullets that point out important limitations, unstated assumptions, oversimplifications, internal tensions, or places where the argument feels under-supported. Keep the critique transcript-grounded and specific. Do not invent outside fact-checks or accuse the speaker of bad intent. The goal is to help the reader understand where confidence should drop, what should be treated cautiously, and what may not generalize cleanly.
...
| `Caveats` | 3-5 complete bullets, each 1-2 sentences | grounded limitations and calibrated skepticism | generic filler caveats, imported outside fact-checking, hostile intent claims, overlong bullets |
