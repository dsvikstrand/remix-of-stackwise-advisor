-- Daily blueprint generation metrics derived from durable generation trace tables.
...
WITH prompt_events AS (
  SELECT
    e.run_id,
    COUNT(*) FILTER (
      WHERE e.event = 'prompt_rendered'
        AND (e.payload ->> 'operation') = 'generateYouTubeBlueprint'
    )::integer AS attempt_count,
    COUNT(*) FILTER (
      WHERE e.event = 'prompt_rendered'
        AND (e.payload ->> 'operation') IN (
          'generateYouTubeBlueprint',
          'generateYouTubeBlueprintPass2Transform'
        )
    )::integer AS output_count
  FROM public.generation_run_events e
  GROUP BY e.run_id
)
SELECT
  r.run_id,
