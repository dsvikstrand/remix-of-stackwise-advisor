...
describe('source subscription sync service', () => {
  it('publishes a ready blueprint immediately when a reusable variant already exists', async () => {
    ...
    const upsertFeedItemWithBlueprint = vi.fn(async () => ({ id: 'ufi_ready_1' }));
    ...
    expect(upsertFeedItemWithBlueprint).toHaveBeenCalledWith(db, {
      userId: 'user_1',
    ...
    expect(ensureSourceItemUnlock).not.toHaveBeenCalled();
  ...
  it('inserts the unlockable feed row before auto-unlock work runs', async () => {
    ...
    const events: string[] = [];
    const insertFeedItem = vi.fn(async () => {
      events.push('insert_feed_item');
      return { id: 'ufi_1' };
    });
    const attemptAutoUnlockForSourceItem = vi.fn(async () => {
      events.push('attempt_auto_unlock');
      return {
        queued: true,
        reason: 'QUEUED',
        job_id: 'job_1',
    ...
    expect(events).toEqual(['insert_feed_item', 'attempt_auto_unlock']);
  ...
  it('treats a recovered-but-unchanged channel id as a transient soft failure and retries once', async () => {
    const db = createMockSupabase({
      user_source_subscriptions: [{
        id: 'sub_1',
        user_id: 'user_1',
        source_type: 'youtube',
        source_channel_id: 'channel_same',
        source_channel_url: 'https://youtube.com/@channel-handle',
        source_channel_title: 'Same Channel',
        source_page_id: 'page_1',
        auto_unlock_enabled: true,
        is_active: true,
        last_polled_at: '2026-03-19T11:00:00.000Z',
    ...
    const resolveYouTubeChannel = vi.fn(async () => ({
      channelId: 'channel_same',
      channelUrl: 'https://youtube.com/channel/channel_same',
  ...
  it('recovers from a transient feed 404 when the channel still resolves and the retry succeeds', async () => {
    const db = createMockSupabase({
      user_source_subscriptions: [{
        id: 'sub_1',
        user_id: 'user_1',
