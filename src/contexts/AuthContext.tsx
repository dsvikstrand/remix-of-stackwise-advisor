...
import { getProfileByUserId, updateMyProfile } from '@/lib/profileApi';
...
interface AuthContextType {
  ...
  updateProfile: (updates: Partial<Pick<Profile, 'display_name' | 'avatar_url' | 'bio' | 'is_public'>>) => Promise<{ error: Error | null }>;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfile = async (userId: string) => {
    try {
      return await getProfileByUserId(userId) as Profile | null;
    } catch (error) {
  ...
  const updateProfile = async (updates: Partial<Pick<Profile, 'display_name' | 'avatar_url' | 'bio' | 'is_public'>>) => {
